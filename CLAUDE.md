...
- **v1 retrieval:** `service/retrievers/vector.py` builds and searches `service/data/index.sqlite` (`build_index.py`, resumable and rate-limited); `rerank.py` is the Voyage cross-encoder. `service/patterns/_generate.py` holds **the one prompt template and the one Claude call every pattern shares** — naive and rerank differ only in which chunks reach it. Never give a pattern its own prompt: the comparison depends on it.
- Only passage text is embedded. Title and author are metadata shown to the generator; no rating, date, page count or shelf is ever in a chunk. That is what makes the numbers questions an honest test.
- **Keys and Python:** `.env` holds `ANTHROPIC_API_KEY` and `VOYAGE_API_KEY`, git-ignored, never committed or printed. The `anthropic` SDK needs Python 3.10+, so model code runs on `.venv/bin/python`; the ingest, the API and the 3.9 rules below are unchanged. Models: `voyage-4`, `rerank-3`, `claude-opus-5` at its defaults.
- `eval/run.py` runs the set and never grades; `eval/grade.py` writes the sheet with a proposed verdict clearly marked as the AI's; `eval/export_replay.py` writes `service/data/replay.json`. **Only a verdict Shirley wrote in the `final` column reaches the site.**
- The live panel **replays** naive and rerank from `replay.json` and runs only the SQL path live. The deployed service therefore needs no model keys — do not add `anthropic` or `voyageai` to `requirements.txt`.
