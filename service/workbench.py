...
One page: the nine questions as buttons (the ledger ones run through answer() with the forced
SQL path; the others say what they wait for), a free SQL box against the read-only tool, and the
full answer() result. Standard library only.
"""
...
QUESTIONS = [
    ...
    ('Q3', 'Why did I give The Fellowship of the Ring two stars when everyone else loved it?', 'waits for the vector index and a generation call (v1, after Kindle)'),
...
.q.off{color:var(--mute);cursor:default}.q.off small{display:block;font-family:-apple-system,Inter,Arial,sans-serif;font-size:11.5px;margin-top:3px}
...
<p class="sub">What exists today: the SQLite ledger, the read-only SQL tool, and the forced SQL path. No model is called yet, so every answer here is a query result, not a pattern.</p>
<div class="grid">
<div>
...
def page(out_html):
    ...
    for qid, text, waits in QUESTIONS:
        if waits:
            ...
            qs += f'<form method="post" action="/ask" style="margin:0"><input type="hidden" name="qid" value="{qid}"><button class="q" type="submit"><b>{qid}</b>{html.escape(text)}</button></form>'
    ...
    def do_POST(self):
        ...
        try:
            if path == '/ask':
                ...
                self._send(page(render_out(result=r, title=f'{qid} · {text}')))
            elif path == '/sql':
                q = form.get('sql', [''])[0]
